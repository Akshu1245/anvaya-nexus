...
def seed_users(repository,password):
    for user_id,username,role,station,district in DEMO_USERS:
        repository.connection.execute("INSERT OR IGNORE INTO users VALUES (?,?,?,?,?,?,1)",(user_id,username,generate_password_hash(password),role,station,district))
    repository.connection.commit()

def login(repository,username,password,ttl,request_id):
    row=repository.connection.execute("SELECT * FROM users WHERE username=? AND active=1",(username,)).fetchone()
    ...
    repository.connection.execute("INSERT INTO sessions VALUES (?,?,?,?,?,NULL)",(sid,row["id"],hashlib.sha256(token.encode()).hexdigest(),now.isoformat(),(now+timedelta(minutes=ttl)).isoformat()));repository.connection.commit()
...
def current_user(repository,token,request_id=None):
    ...
    digest=hashlib.sha256(token.encode()).hexdigest();row=repository.connection.execute("SELECT u.*,s.id session_id,s.expires_at,s.revoked_at FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=?",(digest,)).fetchone()
...
def revoke(repository,token,request_id):
    row=current_user(repository,token,request_id);now=datetime.now(timezone.utc).isoformat();repository.connection.execute("UPDATE sessions SET revoked_at=? WHERE id=?",(now,row["session_id"]));repository.connection.commit();audit(repository,"LOGOUT","SUCCESS",row["id"],request_id,{})
