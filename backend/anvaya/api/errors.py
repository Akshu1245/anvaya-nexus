...
def register_error_handlers(app: Flask) -> None:
    ...
    def handle_api_error(error: ApiError):
        if error.status_code == 403 and getattr(g, "user", None):
            from backend.anvaya.services.audit import audit
            audit(current_app.extensions["repository"], "PERMISSION_DENIAL", "DENIED", g.user["id"], getattr(g, "request_id", None), {"code": error.code, "path": request.path})
