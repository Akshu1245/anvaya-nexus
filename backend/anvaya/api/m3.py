...
from backend.anvaya.services.auth import current_user,login,register,role_login,public_user,revoke
from backend.anvaya.services.policy import evaluate
from backend.anvaya.config import ai_assist_enabled
from backend.anvaya.services.llm import general_ai_answer, is_record_search_query, llm_answer, llm_interpret, templated_answer
...
def auth_login():
 ...
 # UI sends name + role (no password). Route to role-based login.
 if role and not password:
  token,user=role_login(
   current_app.extensions["repository"],
   username,
   role,
   payload.get("station"),
   payload.get("district"),
   current_app.config["SESSION_TTL_MINUTES"],
   g.request_id
  )
 else:
  token,user=login(current_app.extensions["repository"],username,str(password or ""),current_app.config["SESSION_TTL_MINUTES"],g.request_id)
...
@m3_blueprint.post("/auth/register")
def auth_register():
 _check_login_rate()
 payload=request.get_json(silent=True) or {}
 ...
 token,user=register(current_app.extensions["repository"],officer_id,full_name,role,password,station,district,current_app.config["SESSION_TTL_MINUTES"],g.request_id)
 body,status=_ok(user,status=201);response=make_response(body,status);response.set_cookie(current_app.config["SESSION_COOKIE_NAME"],token,httponly=True,samesite="Strict",secure=bool(current_app.config["HTTPS_ENABLED"]),max_age=current_app.config["SESSION_TTL_MINUTES"]*60);return response
...
@m3_blueprint.get("/dashboard/stats")
@protected
def dashboard_stats():
 repo=current_app.extensions["repository"]
 try:
  # Try to get aggregate stats from the repository
  stats=repo.get_dashboard_stats(g.user.get("assigned_district"),g.user.get("assigned_station"))
 except Exception:
  stats={"total_firs":0,"pending":0,"resolved":0,"priority":0,"recent_cases":[],"by_offence":[]}
 return _ok(stats)
...
@m3_blueprint.post("/investigations/<iid>/chat/message")
@protected
def investigation_chat_message(iid):
 repo=current_app.extensions["repository"];inv=_owned(repo,iid);payload=request.get_json(silent=True) or {}
 ...
 if not text:
  raise ApiError("QUERY_REQUIRED","Message text is required.",400,False)

 action_res=resolve_chat_action(text,context)
 if action_res.get("kind")=="action":
  audit(repo,"CHAT_ACTION_RESOLVED","SUCCESS",g.user["id"],g.request_id,{"investigation_id":iid,"action":action_res.get("action"),"case_ref":action_res.get("case_ref")})
  return _ok({"kind":"action","action_result":action_res})

 if is_record_search_query(text):
  plan,engine=_interpret_plan(text,inv["selected_sources"])
  decision=evaluate(g.user,inv["purpose"],plan.selected_sources,"SEARCH",plan.result_limit)
  if not decision.allowed:
   audit(repo,"PERMISSION_DENIAL","DENIED",g.user["id"],g.request_id,decision.dict())
   raise ApiError(decision.denial_code,decision.explanation,403,False)
  results=search_cases(repo,g.user,inv["purpose"],plan)
  answer_payload=llm_answer(text,results,plan,current_app.config) or templated_answer(text,results,plan)
  audit(repo,"AI_CHAT_SEARCH","SUCCESS",g.user["id"],g.request_id,{"investigation_id":iid,"result_count":len(results),"model_used":answer_payload.get("model_used")})
  return _ok({
 ...
 ai_response=general_ai_answer(text,history=history,config=current_app.config)
 audit(repo,"AI_CHAT_MESSAGE","SUCCESS",g.user["id"],g.request_id,{"investigation_id":iid,"model_used":ai_response.get("model_used")})
 return _ok({
  "kind":"answer",
  "answer":ai_response
 })
