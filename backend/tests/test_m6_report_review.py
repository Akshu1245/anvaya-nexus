...
def login(c,u='investigator.demo'):return c.post('/api/auth/login',json={'username':u,'password':P})
def report(c):
 inv=c.post('/api/investigations',json={'title':'M6 report test','purpose':'Active Case Investigation','selected_sources':['CCTNS_REPLICA']}).json['data'];return c.post('/api/reports',json={'title':'Report','investigation_id':inv['id'],'sections':['Cover','Disclaimer'],'notes':'<script>x</script>'}).json['data']
...
def test_m6_production_configuration_cookie_rate_and_request_limits():
 ...
 first=c.post('/api/auth/login',json={'username':'investigator.demo','password':P});assert first.status_code==200 and 'Secure' in first.headers['Set-Cookie'] and 'SameSite=Strict' in first.headers['Set-Cookie']
 second=c.post('/api/auth/login',json={'username':'investigator.demo','password':P});assert second.status_code==429 and second.json['code']=='LOGIN_RATE_LIMITED'
