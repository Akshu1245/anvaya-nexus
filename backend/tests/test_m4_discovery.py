...
def test_home_source_control_and_preset(client,app):
    ...
    sources=client.get("/api/source-control").json["data"]["sources"]
...
def test_followup_discover_case360_passport_and_path(client,app):
    ...
    case=client.get("/api/cases/SYN-CASE-0001/360");assert case.status_code==200 and "trust_issues" in case.json["data"]
    source_id=case.json["data"]["overview"]["source_record_references"][0]
    assert client.get(f"/api/source-passports/{source_id}").status_code==200
    assert client.get("/api/relationships/path?from=SYN-CASE-0001&to=SYN-CASE-0002").status_code==200
