...
def login(client, username="investigator.demo"):
    response = client.post("/api/auth/login", json={"username": username, "password": client.application.config["DEMO_PASSWORD"]})
...
def test_m6_verified_acceptance_flow(client, app):
    ...
    case = client.get(f"/api/cases/{GOLDEN_CASE}/360?purpose=Active%20Case%20Investigation").json["data"]
    source_record = case["overview"]["source_record_references"][0]
    assert client.get(f"/api/source-passports/{source_record}?purpose=Active%20Case%20Investigation").status_code == 200
    for endpoint in (f"/api/m5/case-dna/{GOLDEN_CASE}/{RELATED_CASE}", f"/api/m5/graph/{GOLDEN_CASE}", f"/api/m5/assurance/{GOLDEN_CASE}", f"/api/m5/verify/{GOLDEN_CASE}/{RELATED_CASE}", f"/api/m5/actions/{GOLDEN_CASE}"):
        assert client.get(endpoint + "?purpose=Active%20Case%20Investigation").status_code == 200
    assert client.post(f"/api/m5/challenge/{GOLDEN_CASE}?purpose=Active%20Case%20Investigation", json={"hypothesis": "These cases may involve the same operational group."}).status_code == 200
