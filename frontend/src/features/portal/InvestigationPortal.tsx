...
export function InvestigationPortal({section,onSectionChange}:Props){
 ...
 async function doPreview(raw=query){
  const text=raw.trim();if(!text){setError(locale==='kn'?'ಪ್ರಶ್ನೆ ನಮೂದಿಸಿ ಅಥವಾ ಕೆಳಗಿನ ಫಿಲ್ಟರ್‌ಗಳನ್ನು ಬಳಸಿ.':'Enter a question or fill the filters below.');return}
 ...
 const queryInvalid=Boolean(error)&&!query.trim()&&!hasVisibleFilters
 ...
 return <div className="grid gap-5">
   ...
   <div>
    <div className="flex flex-wrap items-center gap-2">
     ...
     <span className="rounded-full bg-teal-50 px-2 py-0.5 text-[11px] font-semibold text-teal-800 ring-1 ring-teal-200">
      🤖 AI Engine ({health?.ai_assist_enabled ? 'Llama 3.3 / Gemma Active' : 'Hybrid Rule Fallback'})
     </span>
    </div>
  ...
  <div className="z-30 sm:sticky sm:top-8 sm:rounded-xl sm:bg-[#eef2f4]/95 sm:py-2 sm:backdrop-blur"><JourneyStepper current={stage} maxReached={maxStage} onSelect={onJourney}/></div>
  ...
  {workspaceSection==='search'&&!isSupervisor&&<section className="grid gap-5" aria-label="Search workspace">
   <aside className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-teal-200 bg-teal-50 p-4">
    <div><p className="font-semibold text-teal-950">{locale==='kn'?'ಹೊಸದಾಗಿ ಬಂದಿರಾ? ಸಿದ್ಧಪಡಿಸಿದ ಡೆಮೊ ಪ್ರಶ್ನೆಯಿಂದ ಪ್ರಾರಂಭಿಸಿ.':'New here? Start with the prepared demo query.'}</p><p className="text-xs text-teal-800">{locale==='kn'?'ಇದು ವ್ಯಾಖ್ಯಾನವನ್ನು ಮಾತ್ರ ಸಿದ್ಧಪಡಿಸುತ್ತದೆ; Search records ಒತ್ತುವವರೆಗೆ ಯಾವುದೇ ದಾಖಲೆ ಪಡೆಯುವುದಿಲ್ಲ.':'This prepares the interpretation only; no records are retrieved until you press Search records.'}</p></div>
   ...
   <aside className="rounded-2xl border border-slate-200 bg-white px-4 py-3" aria-label="ANVAYA safeguards">
    <div className="flex flex-wrap items-center gap-2 text-xs"><b className="mr-1 text-navy-950">{locale==='kn'?'ರಕ್ಷಣೆಗಳು ಸದಾ ಸಕ್ರಿಯ':'Guardrails always on'}</b><span className="rounded-full bg-slate-100 px-2 py-1">{locale==='kn'?'ಸಿಂಥೆಟಿಕ್ ಡೇಟಾ ಮಾತ್ರ':'Synthetic data only'}</span><span className="rounded-full bg-slate-100 px-2 py-1">{locale==='kn'?'ಮಾನವ ದೃಢೀಕರಣ ಅಗತ್ಯ':'Human confirmation required'}</span><span className="rounded-full bg-slate-100 px-2 py-1">{locale==='kn'?'ತೀರ್ಪು ಅಥವಾ ಅಪಾಯ ಸ್ಕೋರ್ ಇಲ್ಲ':'No guilt or risk scoring'}</span></div>
   </aside>
   <div className="rounded-2xl border bg-white p-5 shadow-sm">
    <p className="text-xs font-bold uppercase tracking-[.14em] text-teal-700">{locale==='kn'?'ಹಂತ 1 · ಕೇಳಿ':'Step 1 · Ask'}</p>
    ...
    <label className="mt-4 block text-sm font-medium">{locale==='kn'?'ಪ್ರಶ್ನೆ (ಐಚ್ಛಿಕ)':'Question (optional)'}
     <textarea aria-label="Investigation question" aria-invalid={queryInvalid||undefined} aria-describedby="query-guidance" className={`mt-1 min-h-20 w-full rounded-xl border p-3 transition focus:ring-2 focus:ring-teal-300 ${queryInvalid?'border-red-300 bg-red-50/40':'border-slate-200'}`} value={query} onChange={e=>{setQuery(e.target.value);setPreview(null)}} placeholder={locale==='kn'?'ಉದಾ. ಬಗೆಹರಿಯದ ಸರಗಳ್ಳತನ SYN-STN-01':'e.g. unresolved chain snatching near SYN-STN-01'}/>
     <span id="query-guidance" className="mt-1 block text-xs font-normal text-slate-500">{locale==='kn'?'ಪ್ರಶ್ನೆಯನ್ನು ನಮೂದಿಸಿ ಮತ್ತು ಕೆಳಗಿನ ಹುಡುಕಾಟ ಬಟನ್ ಒತ್ತಿ, ಅಥವಾ ನೇರ ಫಿಲ್ಟರ್‌ಗಳನ್ನು ಬಳಸಿ.':'Type a question and press Search records below, or use the direct filters.'}</span>
     {queryInvalid&&<span className="mt-1 block text-xs font-normal text-red-700">{locale==='kn'?'ಪ್ರಶ್ನೆ ನಮೂದಿಸಿ ಅಥವಾ ಕೆಳಗಿನ ಫಿಲ್ಟರ್‌ಗಳನ್ನು ಬಳಸಿ.':'Enter a question or set a filter below.'}</span>}
    </label>
   ...
   <div className="rounded-2xl border bg-white p-5 shadow-sm">
    <p className="text-xs font-bold uppercase tracking-[.14em] text-teal-700">{locale==='kn'?'ಹಂತ 2 · ವ್ಯಾಪ್ತಿಯನ್ನು ಹೊಂದಿಸಿ':'Step 2 · Set the scope'}</p>
    ...
    <fieldset className="mt-3 grid gap-x-4 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
     {([['offence','Offence'],['status','Status'],['location','Location / station'],['crime_number','Crime number'],['case_number','Case number'],['date_from','Incident from'],['date_to','Incident to'],['person_name','Person name'],['person_role','Person role'],['act_code','Act code'],['section_code','Section code'],['police_unit','Police unit'],['district','District']] as const).map(([key,label])=>{
      ...
      return <label key={key} className="flex flex-col text-sm"><span className="mb-1 font-medium text-slate-600">{visibleLabel}</span><input aria-label={visibleLabel} className={`w-full rounded-lg border p-2 transition focus:ring-2 focus:ring-teal-300 ${active?'border-teal-400 bg-teal-50/40':'border-slate-200'}`} value={(filters as any)[key]} onChange={e=>setFilters({...filters,[key]:e.target.value})}/></label>
     })}
   ...
   {preview&&<div className="grid gap-2"><p className="text-xs font-bold uppercase tracking-[.14em] text-teal-700">{locale==='kn'?'ಹಂತ 3 · ಪರಿಶೀಲಿಸಿ':'Step 3 · Review the interpretation'}</p><QueryInterpretationPanel preview={preview} onChange={setPreview}/></div>}

   <div className="rounded-2xl border border-teal-200 bg-white p-4 shadow-sm" aria-label="Search actions">
    <p id="search-action-help" className="mb-3 text-sm text-slate-600">{preview
     ?(locale==='kn'?'ವ್ಯಾಖ್ಯಾನ ಸಿದ್ಧವಾಗಿದೆ. ದಾಖಲೆಗಳನ್ನು ಹುಡುಕುವ ಮೊದಲು ಅದನ್ನು ಮೇಲಿಂದ ಪರಿಶೀಲಿಸಿ.':'Interpretation ready. Review it above, then retrieve the authorised records.')
     :hasVisibleFilters
      ?(locale==='kn'?'ಗೋಚರ ಫಿಲ್ಟರ್‌ಗಳು ಸಿದ್ಧವಾಗಿವೆ. ಹುಡುಕಾಟವು ಈ ವ್ಯಾಪ್ತಿಯನ್ನು ಮಾತ್ರ ಬಳಸುತ್ತದೆ.':'Visible filters are ready. Search will use only this displayed scope.')
      :(locale==='kn'?'ಪ್ರಶ್ನೆಯನ್ನು ಪೂರ್ವವೀಕ್ಷಿಸಿ ಅಥವಾ ಕನಿಷ್ಠ ಒಂದು ಫಿಲ್ಟರ್ ಹೊಂದಿಸಿ.':'Preview your question or set at least one filter to enable Search records.')}</p>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
     <button type="button" aria-describedby="search-action-help" className={`${btnPrimary} order-1 w-full justify-center sm:order-2 sm:w-auto`} disabled={Boolean(busy)||!searchReady} onClick={()=>void runSearch()}>{busy==='search'?t('loading'):<>{t('searchRecords')} <span aria-hidden>→</span></>}</button>
      ...
      {results.map((item:any)=><article key={item.case_id||item.id} className="rounded-xl border border-slate-200 p-4 transition hover:border-teal-400 hover:shadow-md">
       <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
         ...
         <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]"><span className="rounded-full bg-slate-100 px-2 py-1">{locale==='kn'?'ಮೂಲ:':'Source:'} {item.source_system||item.source_system_id||selected[0]||'CCTNS_REPLICA'}</span><span className="rounded-full bg-teal-50 px-2 py-1 text-teal-800">{locale==='kn'?'ದೃಢೀಕರಿಸಿದ ವ್ಯಾಪ್ತಿಗೆ ಹೊಂದಿದೆ':'Matched confirmed scope'}</span>{typeof preview?.normalised_interpretation?.confidence==='number'&&<span className="rounded-full bg-slate-100 px-2 py-1">{locale==='kn'?'ಪ್ರಶ್ನೆ ವ್ಯಾಖ್ಯಾನ:':'Query interpretation:'} {Math.round(preview.normalised_interpretation.confidence*100)}%</span>}</div>
        </div>
       ...
       <p className="mt-2 text-xs text-slate-500">{locale==='kn'?'ಸಂಬಂಧಿತ ಪ್ರಕರಣಗಳು, ಗ್ರಾಫ್, ಕ್ಲಸ್ಟರ್‌ಗಳು ಮತ್ತು ವರದಿಗಾಗಿ Case 360 ತೆರೆಯಿರಿ.':'Open Case 360 for related cases, graph, network clusters, priorities and the cited dossier.'}</p>
      </article>)}
     </div>}
   </div>
  </section>}
  ...
  {workspaceSection==='chat'&&<section className="rounded-2xl border bg-white p-5 shadow-sm" aria-label="Chat assist">
   ...
   <div className="mt-3 flex flex-wrap gap-2">
    <input aria-label="Ask ANVAYA" className="min-w-0 flex-[1_1_14rem] rounded-xl border px-3 py-2" value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void runChat()}} placeholder={locale==='kn'?'ಕನ್ನಡ ಅಥವಾ ಇಂಗ್ಲಿಷ್‌ನಲ್ಲಿ ಕೇಳಿ…':'Ask in English or Kannada…'}/>
    <button type="button" className={`${btnSecondary} shrink-0`} onClick={()=>void startVoice()} aria-label={recording?'Stop voice recording':'Start voice input'}>{recording?'Stop':'Voice'}</button>
    <button type="button" className={`${btnPrimary} shrink-0`} disabled={Boolean(busy)} onClick={()=>void runChat()}>{t('send')}</button>
   </div>
  </section>}
