...
export function SourcePassportDrawer({passport,onClose}:{passport:any;onClose:()=>void}){
 ...
 return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/45 p-3" role="presentation" onMouseDown={onClose}>
  <aside ref={panelRef} className="h-full w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-label="Source Passport" onMouseDown={event=>event.stopPropagation()}>
