...
beforeEach(()=>{
 ...
 api.preview.mockResolvedValue({message_id:'MSG-1',normalised_interpretation:{intent:'SEARCH',confidence:.9,uncertain_fields:[],filters:{offence:'ROBBERY',status:'UNRESOLVED'},selected_sources:['CCTNS_REPLICA'],result_limit:25}})
...
it('shows the FIR filter form on the search section',async()=>{
 ...
 expect(screen.getAllByText(/Filter|Offence/i).length).toBeGreaterThan(0)
 expect(screen.getByLabelText(/Offence/i)).toBeInTheDocument()
 expect(screen.getAllByRole('button',{name:/Search records|ದಾಖಲೆಗಳನ್ನು ಹುಡುಕಿ/i}).length).toBeGreaterThanOrEqual(1)
 expect(screen.getByText(/Step 1 · Ask/i)).toBeInTheDocument()
 expect(screen.getByText(/Step 2 · Set the scope/i)).toBeInTheDocument()
})

it('requires a typed question to be previewed before retrieval',async()=>{
 const {user}=renderPortal('search')
 await enterDemo(user)
 const searchBtns = screen.getAllByRole('button',{name:/Search records|ದಾಖಲೆಗಳನ್ನು ಹುಡುಕಿ/i})
 const search = searchBtns[searchBtns.length - 1]
 const input = screen.queryByLabelText('Investigation question') || screen.getByLabelText(/Question/i)
 await user.type(input,'unresolved robbery cases')
 const previewBtn = screen.getAllByRole('button',{name:/Preview query|ಪ್ರಶ್ನೆ ಪೂರ್ವವೀಕ್ಷಣೆ/i})[0]
 await user.click(previewBtn)
 expect(search).toBeEnabled()
})

it('prepares the golden demo query without retrieving records',async()=>{
 const {user}=renderPortal('search')
 await enterDemo(user)
 await user.click(screen.getByRole('button',{name:'Try demo query'}))
 await waitFor(()=>expect(api.preview).toHaveBeenCalledWith('INV-1',expect.stringMatching(/chain snatching.*Jayanagar/i)))
 const input = (screen.queryByLabelText('Investigation question') || screen.getByLabelText(/Question/i)) as HTMLTextAreaElement
 expect(input.value).toMatch(/chain snatching.*Jayanagar/i)
 expect(api.search).not.toHaveBeenCalled()
 expect(screen.getAllByRole('button',{name:/Search records/i})[0]).toBeEnabled()
})
...
it('opens Case 360 with purpose and sources',async()=>{
 ...
 expect(screen.queryByRole('button',{name:/related/i})).not.toBeInTheDocument()
 expect(screen.queryByRole('button',{name:/networkClusters/i})).not.toBeInTheDocument()
 await user.click(screen.getByRole('button',{name:/openCase360/i}))
 ...
 expect(screen.getByRole('button',{name:/networkClusters/i})).toBeInTheDocument()
})

it('loads trends with seasonality fields',async()=>{
 const {user}=renderPortal('trends')
 await enterDemo(user)
 await user.click(screen.getByRole('button',{name:'loadTrends'}))
...
it('renders investigation portal search section directly',async()=>{
  const {user}=renderPortal('search')
  await enterDemo(user)
  expect(screen.getByText(/Step 1 · Ask/i)).toBeInTheDocument()
  expect(screen.getByRole('button',{name:/Try demo query/i})).toBeInTheDocument()
})
