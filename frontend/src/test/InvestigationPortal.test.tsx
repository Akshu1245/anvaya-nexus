...
it('shows the FIR filter form on the search section',async()=>{
 ...
 expect(screen.getAllByRole('button',{name:/Search records|ದಾಖಲೆಗಳನ್ನು ಹುಡುಕಿ/i})).toHaveLength(2)
 expect(screen.getAllByRole('button',{name:/Search records|ದಾಖಲೆಗಳನ್ನು ಹುಡುಕಿ/i})[1]).toBeDisabled()
...
it('requires a typed question to be previewed before retrieval',async()=>{
 const {user}=renderPortal('search')
 await enterDemo(user)
 const search=screen.getAllByRole('button',{name:/Search records|ದಾಖಲೆಗಳನ್ನು ಹುಡುಕಿ/i})[1]
 await user.type(screen.getByLabelText('Investigation question'),'unresolved robbery cases')
 expect(search).toBeDisabled()
 await user.click(screen.getAllByRole('button',{name:/Preview query|ಪ್ರಶ್ನೆ ಪೂರ್ವವೀಕ್ಷಣೆ/i})[0])
 expect(await screen.findByText(/Step 3 · Review the interpretation/i)).toBeInTheDocument()
...
it('prepares the golden demo query without retrieving records',async()=>{
 ...
 expect((screen.getByLabelText('Investigation question') as HTMLTextAreaElement).value).toMatch(/chain snatching.*Jayanagar/i)
...
it('opens Case 360 with purpose and sources',async()=>{
 ...
 expect(screen.queryByRole('button',{name:/Related cases|ಸಂಬಂಧಿತ ಪ್ರಕರಣಗಳು/i})).not.toBeInTheDocument()
 expect(screen.queryByRole('button',{name:/Network clusters|ನೆಟ್‌ವರ್ಕ್ ಕ್ಲಸ್ಟರ್‌ಗಳು/i})).not.toBeInTheDocument()
 ...
 expect(screen.getByRole('button',{name:/Related cases|ಸಂಬಂಧಿತ ಪ್ರಕರಣಗಳು/i})).toBeInTheDocument()
 expect(screen.getByRole('button',{name:/Network clusters|ನೆಟ್‌ವರ್ಕ್ ಕ್ಲಸ್ಟರ್‌ಗಳು/i})).toBeInTheDocument()
})
