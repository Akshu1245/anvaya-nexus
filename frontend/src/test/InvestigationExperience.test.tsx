...
it('renders the authenticated investigation entry point',async()=>{
  ...
  expect(screen.getByRole('heading',{name:'Start the guided ANVAYA demo'})).toBeInTheDocument()
  expect(await screen.findByRole('button',{name:'Open public demo'})).toBeInTheDocument()
  expect(screen.getByText(/nothing retrieves records until you confirm Search/i)).toBeInTheDocument()
  expect(screen.getAllByText('30')).not.toHaveLength(0)
  expect(screen.getByText('Synthetic FIRs')).toBeInTheDocument()
  expect(screen.getByRole('region',{name:'Portal capabilities'}).querySelectorAll('svg')).toHaveLength(6)
  const demoSection=screen.getByRole('heading',{name:'Start the guided ANVAYA demo'}).closest('section')!
  expect(demoSection.compareDocumentPosition(screen.getByRole('region',{name:'Portal capabilities'}))&Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
})
