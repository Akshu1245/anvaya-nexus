...
export function useChat() {
  ...
  const sendMessage = useCallback(
    async (raw?: string) => {
      ...
      const resolved = await m3Api
        .resolveChatAction(investigation.id, text, context)
        .catch(() => null)
      ...
      if (resolved?.kind === 'action') {
        ...
        if (resolved.action === 'BRIEFING') {
          ...
          chatStore.setActiveCaseId(caseRef)
          const data = await call('network-clusters', () =>
            m3Api.networkClusters(investigation.id, caseRef),
          )
          if (data) {
            ...
            const summary = clusters.length
              ? clusters
                  .map(
                    (c: any) =>
                      `${(c.member_case_ids || []).join(', ')} — Candidate factual connection based on authorised stored relationship records only; human verification required.`,
                  )
                  .join('\n')
              : 'No candidate factual clusters were found for this case in the authorised synthetic records.'
            chatStore.addMessage({ id: uid(), role: 'assistant', kind: 'text', text: summary })
          ...
          chatStore.addMessage({
            ...
            role: 'assistant',
            kind: 'text',
            text: resolved.message || 'That guided action is not available in this prototype.',
          })
        }
        return
      }
      ...
      const value = await call('ask', () =>
        chatStore.parentMessageId
          ? m3Api.followUp(investigation.id, chatStore.parentMessageId, text)
          : m3Api.preview(investigation.id, text),
      )
      ...
      if (!value) {
        chatStore.addMessage({
          ...
          role: 'assistant',
          kind: 'text',
          text: 'Could not interpret that question. Try naming an offence, a place, a status, or a date range.',
        })
        return
      }

      const history = await m3Api.history(investigation.id).catch(() => [])
      chatStore.setParentMessageId(history.at(-1)?.id || '')
      chatStore.addMessage({ id: uid(), role: 'assistant', kind: 'interpretation', preview: value })
      invStore.setPreview(value)
    },
